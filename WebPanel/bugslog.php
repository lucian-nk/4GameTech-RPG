...
if(!empty($w)){
	...
	$query = mysql_query('SELECT * FROM BugsLog '.$w.' ORDER BY id DESC') or die(mysql_error());
	...
	$query = mysql_query('SELECT * FROM BugsLog ORDER BY id DESC LIMIT '.(($pagenum-1)*$rows_per_page).', '.$rows_per_page) or die(mysql_error());
	while($row = mysql_fetch_array($query)){
		echo '<tr>
			<td>'.$row['id'].'</td>
			<td>'.$row['Text'].'</td>
