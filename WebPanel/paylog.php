...
<center><form method="GET" action="paylog.php">
...
if(!empty($w)){
	...
	$query = mysql_query('SELECT * FROM paylog '.$w.' ORDER BY id DESC') or die(mysql_error());
...
echo "</div>";
	...
	$search = '';
	$total_rows = get_row("SELECT COUNT(id) as total FROM paylog ORDER BY id DESC");
	...
	$query = mysql_query('SELECT * FROM paylog ORDER BY id DESC LIMIT '.(($pagenum-1)*$rows_per_page).', '.$rows_per_page) or die(mysql_error());
