...
foreach($member_types as $id => $member){
	if($id > 0){
		$tmp[] = '<a class="active" href="complaints.php?f='.$id.'">'.$member.'</a>';
	}
} ?>
...
<a href="search.php" class="btn btn-danger">New Complaint</a>
</div>
<br>
...
<?php $query = mysql_query('SELECT * from complaints WHERE factionid  = 0 order by id desc'); 
